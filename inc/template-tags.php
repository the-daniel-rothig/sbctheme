...
if ( ! function_exists( 'sbctheme_entry_date' ) ) :
/**
 * Prints HTML with date information for current post.
 *
 * Create your own twentysixteen_entry_date() function to override in a child theme.
...
function sbctheme_entry_date() {
	...
	printf( '<span class="posted-on"><span class="screen-reader-text">%1$s </span>%3$s</span>',
		_x( 'Published on', 'Used before publish date.', 'twentysixteen' ),
		esc_url( get_permalink() ),
		$time_string
	);
}
...
if( !function_exists('sbctheme_queryashiddeninputs') ) :
function sbctheme_queryashiddeninputs() {
...
if ( !function_exists('sbctheme_ticketstatus') ) :
function sbctheme_ticketstatus($ticketstatus_result) {
	if (!$ticketstatus_result['isError']) {
				$ref = $ticketstatus_result['ticket']['ref'];
				$status = $ticketstatus_result['ticket']['workflow']['title'];

				echo "<h1 class='heading-xlarge'>Complaint status</h1>";
				echo "<p>Complaint reference: $ref</p>";
				
				$date = date_i18n('j F, Y', $ticketstatus_result['ticket']['date_created_ts']); 						
				echo "<p>Investigation started on $date</p>";
		
				
				$query = new WP_Query(array(
						'pagename' => 'sbc-complaints/'. str_replace(" ", "-", strtolower($status)),
						'post_type' => 'page'
					));
				if ($query->have_posts()) {
					...
					echo "<p class='ticketstatusmessage'>The current status of this complaint is: <span class='bigstatus $status'>$status</span></p>";
				}


				echo "<h3 class='heading-medium'>Status history:</h3><ol class='list'>";
				foreach ($ticketstatus_result['log']['logs'] as $log) {
					if ($log['action_type'] === 'changed_workflow') {
						$status = $log['details']['new_workflow_title'];
						$date = date_i18n('j F, Y', $log['date_created_ts']); 
						echo "<li class='timeline $status'><span class='timeline-blob'></span>$date: <strong>$status</strong></li>";
					}
				}
...
if( !function_exists('sbctheme_ticketcreationsuccess') ) :
function sbctheme_ticketcreationsuccess($ticket_result) {
	$href = esc_url( get_the_permalink() . '?ticket='. $ticket_result['data']['ref']);
	$linktext = esc_html($ticket_result['data']['ref']);
	echo "<div class='govuk-box-highlight'><h2 class='bold-large'>You have submitted a complaint</h2><p>Your reference number is:<br><span class='heading-medium'><a href='$href'>$linktext</a></span></p></div>";
}
endif;
